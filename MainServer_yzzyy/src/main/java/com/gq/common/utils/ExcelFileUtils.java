...
public class ExcelFileUtils {
    ...
    public static void exportExcel(OutputStream os, List<String> fieldNameList, List<ArrayList<String>> fieldDataList) {
        try {
            HSSFWorkbook workBook = createWorkbook(fieldNameList, fieldDataList);
            workBook.write(os);
        } catch (IOException e) {
     ...
     */
    public static HSSFWorkbook createWorkbook(List<String> fieldNameList, List<ArrayList<String>> fieldDataList) throws IOException {
    	// 创建一个工作簿
        ...
        // 创建数据栏单元格并填入数据
        createFieldValue(workBook, sheet, fieldDataList);
        return workBook;
    }
     ...
     * @param fieldDataList <br />
     */
    private static void createFieldValue(HSSFWorkbook workBook, HSSFSheet sheet, List<ArrayList<String>> fieldDataList) {
        int rowNum = 1;
        // 创建数据栏单元格并填入数据
